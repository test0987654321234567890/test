...
[{"type": "flex","altText":"あいうえお","contents":{
    ...
    "body": {
        ...
        <div id="uniPad"></div>
            ...
            function tashiro(){
                saveApp();
                button1.innerHTML = `<button type="button" onclick= url()>通報リンク変換</button><button type="button" onclick= uni()>ユニコ自動連投</button><button type="button" onclick= flex() >Flexメッセージ生成</button><button type="button" onclick= help()>使い方/ヘルプ</button>`;
                app.innerHTML = flexTool;
                apptype=4;
                setApp();
            }
