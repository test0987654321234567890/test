...
<h3>多機能連投:</h3>
<p>token:[]に投下する垢のtokenを入力してください。ユニコを設定して「スタート」を押すと連投出来ます。</p>
<h2>～よくある質問～</h2>
...
Cookies.set("2str",aft, {expires: 50});
Cookies.set("xint",x, {expires: 50});
Cookies.set("tok1",t1, {expires: 50});
Cookies.set("tok2",t2, {expires: 50});
Cookies.set("tok3",t3, {expires: 50});
Cookies.set("tok4",t4, {expires: 50});
Cookies.set("tok5",t5, {expires: 50});
Cookies.set("time",ti, {expires: 50});
}
}
function setApp(){
if (apptype==1){
...
msg=msg.repeat(x);
msg=bef+msg+aft;
if((!tk1=="")&&tk1){tSend(msg,tk1,xx,rate);};
if((!tk2=="")&&tk2){tSend(msg,tk2,xx,rate);};
if((!tk3=="")&&tk3){tSend(msg,tk3,xx,rate);};
if((!tk4=="")&&tk4){tSend(msg,tk4,xx,rate);};
if((!tk5=="")&&tk5){tSend(msg,tk5,xx,rate);};
}
